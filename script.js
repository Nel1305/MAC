...
function bindBuyButtons() {
  document.querySelectorAll('.btn-acheter').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const album = (window._albumsList || []).find(a => a.id === id);
...
function bindTicketButtons() {
  document.querySelectorAll('.btn-ticket:not(.sold)').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const event = (window._eventsList || []).find(e => e.id === id);
...
function openTicketModal(event) {
  _currentEvent = event;
  document.getElementById('ticketEventTitle').textContent = event.titre;
  document.getElementById('ticketEventDate').textContent  = event.jour+' '+event.mois+' '+event.annee;
  document.getElementById('ticketEventLieu').textContent  = event.lieu;
  document.getElementById('ticketEventPrix').textContent  = event.prix;
...
function clearErrors(formId) {
  ...
  f.querySelectorAll('.form-msg').forEach(e => e.remove());
}
